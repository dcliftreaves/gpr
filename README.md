...
| **4. PSF-aware video improvement** | 4K cleanup and 8K reconstruction that model the real blur from Bayer resize/capture instead of generic sharpening. | Approved 4K cleanup and 8K SR baselines are available for post; a focused Mission42/Z8 PSF-detail continuation improves the current diagnostic candidate, but controlled high/low PSF captures and a PSF-conditioned production gate remain open. |
...
Current four-pillar completion is **70%**. This is a production-readiness burn-down, not an image-quality score, and not a regression signal for locked artifacts.
...
| **3. Premium still/SR** | Offline still processing can spend real compute for the best possible editable raw output, including 4K/8K reconstruction and still latitude recovery. | Routed X2D/Z8/Mission specialists, raw-CFA targets, full-frame reviews, editor-openability receipts, a 13-scene / 351-row expanded target set, raw-target SNR/distribution audits, and deduplicated RCAB/NAF/U-Net teacher receipts. | Current raw-residual models are diagnostic, not production: simple RCAB/NAF scale-up still fails X2D, noise-floor-only weighted U-Net is the best current X2D-scene branch at **0.153%**, stored-HF/pyramid/target-energy/Fourier-band/candidate-scale/source-HF/frame-context follow-ups regress, and the hard holdout has **3.45x** train-median target energy, so the next teacher needs a different noise-aware/camera-aware objective. |
| **4. PSF-aware video/SR** | 4K cleanup and 8K reconstruction should account for the actual blur from Bayer resize/capture instead of guessing. | Approved 4K cleanup and 8K SR baselines, 1,024-tile detail budget, native Mission 1 high/low inventory, measurement plan, first PSF measurement run, kernel-stability audit, a hash-strict controlled-capture request, metric-bearing Mission42/Z8 same-cell detail reruns, and a focused hard-row continuation that improves Mission/Z8 medians without cross-camera collapse. | Current near-time pairs are diagnostic only: 2 accepted pairs, max normalized-weight std **0.809** against a **0.10** gate, and one accepted pair with invalid negative weights. Controlled high/low pairs still need decoded Bayer hashes and stricter fixed-setting provenance. The current focused model still misses the Mission floor on `GP017346`/`GP017600`/`GP017347`/`GP017348`; a stronger loss-weight-only follow-up regressed the worst row, so the next model needs better candidate-side detail/phase/context conditioning or a stronger PSF-aware teacher. |
...
| RAW video PSF/SR improvement | **48%** | Same-cell detail metrics and a focused Mission42/Z8 continuation now improve the diagnostic candidate; the Mission worst-row floor and formal PSF-aware replacement remain open. |
...
|---|---|
| What is done, percent-wise, across the four pillars? | [`docs/PRODUCT_PILLAR_SCORECARD.md`](docs/PRODUCT_PILLAR_SCORECARD.md) and `/Volumes/OWC_8TB/gpr_work/artifacts/product_pillar_scorecard_psf_gradient_focus_20260701/index.html` |
...
| What work remains before calling the whole suite production-ready? | [`docs/BIG_EFFORTS_STATUS.md`](docs/BIG_EFFORTS_STATUS.md), [`docs/HIGH_LEVEL_GOAL_EXECUTION_PLAN.md`](docs/HIGH_LEVEL_GOAL_EXECUTION_PLAN.md), [`docs/PRODUCTION_CAPTURE_REQUIREMENTS.md`](docs/PRODUCTION_CAPTURE_REQUIREMENTS.md), [`docs/PRODUCTION_CAPTURE_REQUIREMENTS.json`](docs/PRODUCTION_CAPTURE_REQUIREMENTS.json), and the requirement-linked burn-down at `/Volumes/OWC_8TB/gpr_work/artifacts/product_burndown_psf_gradient_focus_20260701/index.html` |
...
| Four-pillar product scorecard | [`docs/PRODUCT_PILLAR_SCORECARD.md`](docs/PRODUCT_PILLAR_SCORECARD.md) and `/Volumes/OWC_8TB/gpr_work/artifacts/product_pillar_scorecard_psf_gradient_focus_20260701/index.html` |
| Four-pillar production burn-down | `/Volumes/OWC_8TB/gpr_work/artifacts/product_burndown_psf_gradient_focus_20260701/index.html` |
...
| Raw-stills capture request | `/Volumes/OWC_8TB/gpr_work/artifacts/stills_capture_request_noise_fullmanifest_20260701/index.html` |
