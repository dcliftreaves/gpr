...
| **Premium still/SR** | **60%** | Raw-CFA targets, routed specialists, model-promotion tooling, the 124-receipt experiment scoreboard, X2D/Z8 source-evidence audits, rejected clean-source/frequency-pyramid/gated-residual/masked-detail/raw-CFA source-frequency/raw-CFA residual-signal/candidate-HF no-op/route-conditioned noise-aware smoke receipts, a target/degradation blocker receipt, a replacement target/source contract, Gate 10 source/degradation decision, Gate 11 degradation-source audit, route-specialist readiness audit, routed rendered EV-stress proxy review, and 4/4 editor/latitude coverage now exist. | Route-specialist full-frame metrics are positive. Editable DNG/GPR openability and non-oracle raw-editor latitude are proven for Mission 1 DNG, Mission 1 GPR, Z8 DNG, and X2D DNG. The exact-sidecar-only noise policy passes, but no model receipt clears the 15% / 15% promotion floor or production submission validation. Next work is a Gate 11 candidate intake using the route-isolated teacher/router policy: train X2D signal/mixed rows, default Z8 noise-floor rows to no-op, and forbid another generic residual smoke. |
...
| **Premium still/SR** | 124 runtime-safe experiment receipts, raw-CFA targets, routed specialists, promotion tooling, X2D/Z8 source-evidence audits, rejected clean-source/frequency-pyramid/gated-residual/masked-detail/route-conditioned noise-aware receipts, Gate 10 source/degradation decision, Gate 11 degradation-source audit, route-specialist readiness evidence, routed rendered EV-stress proxy evidence, 4/4 non-oracle editor/latitude coverage, and exact-sidecar clean-target policy exist. | Current candidates are not promotable; next evidence is `premium_still_sr_gate11_candidate_intake_<date>` using the route-isolated teacher/router policy, followed only by paired smoke, the 15% / 15% held-out recovery floor, and production submission validation. |
