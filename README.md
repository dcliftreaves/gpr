...
| **3. Premium still/SR** | Offline still processing can spend real compute for the best possible editable raw output, including 4K/8K reconstruction and still latitude recovery. | Routed X2D/Z8/Mission specialists, raw-CFA targets, full-frame reviews, editor-openability receipts, a 13-scene / 351-row expanded target set, raw-target SNR/distribution audits, PSF/kernel-conditioned trainer plumbing, a PSF metadata gap audit, and deduplicated RCAB/NAF/U-Net teacher receipts. | Current raw-residual models are diagnostic, not production: simple RCAB/NAF scale-up still fails X2D, noise-floor-only weighted U-Net is the best current X2D-scene branch at **0.153%**, first near-box PSF probes reach **0.106%** and **0.064%**, the dedup target has **0/117** rows with row-level PSF metadata, stored-HF/pyramid/target-energy/Fourier-band/candidate-scale/source-HF/frame-context follow-ups regress, and the hard holdout has **3.45x** train-median target energy, so the next teacher needs real per-row PSF/camera variation or a different noise-aware/camera-aware objective. |
...
| What proves or blocks premium still/SR? | [`docs/PREMIUM_STILL_SR.md`](docs/PREMIUM_STILL_SR.md), the raw-CFA residual gap dashboard under `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_raw_cfa_residual_gap_20260701/`, the PSF metadata gap audit at `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_psf_metadata_gap_20260701/index.html`, and the next-experiment contract at `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_next_experiment_contract_20260701/index.html` |
