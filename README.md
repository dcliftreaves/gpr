...
| **3. Spend-time-for-quality still/SR** | Matched 1x CNN improves compressed still latitude; current 4K cleanup and 8K SR infrastructure proves the offline CNN path can emit editable raw, `.gvid`, DNG/GPR, and ProRes review artifacts; routed X2D, Z8, and Mission 1 premium still-SR specialists now have hashed fixtures, full-frame receipts, rendered proxy review, and an X2D 100MP editor-openability plus metadata-transplant receipt. | Real "amazing still" promotion is still open: full raw-editor latitude receipts, X2D center-crop exposure-stress closure, and noise-aware targets before the routed still-SR suite can be called production. |
...
| Premium still-SR | Routed X2D, Z8, and Mission 1 specialists prove the camera/source-specific direction; X2D now has editable DNG/GPR openability plus source-camera metadata transplant, but full raw-editor latitude remains a promotion gate. |
