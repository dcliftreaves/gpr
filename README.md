...
| **3. Spend-time-for-quality still/SR** | Matched 1x CNN improves compressed still latitude; current 4K cleanup and 8K SR infrastructure proves the offline CNN path can emit editable raw, `.gvid`, DNG/GPR, and ProRes review artifacts; routed X2D, Z8, and Mission 1 premium still-SR specialists now have hashed fixtures, full-frame receipts, rendered proxy review, X2D 100MP editor-openability plus metadata transplant, an X2D rawpy latitude dashboard, structured HF residual targets, a blocker audit, and an executed target expansion: **13 scenes / 351 rows** across X2D and Z8 with validated noise sidecars. Raw-CFA target coverage is now complete across that expanded target, the raw-CFA gated model beats matched RGB ablations on expanded Z8 and X2D scene holdouts, a matched dilated raw-CFA gate has been tested, and a calibrated noise-clean target sweep now guards the next target variant. | Real "amazing still" promotion is still open. The raw-CFA gated model improves the direction but remains far below promotion: best broad holdout is **2.92%** median MAE recovery against a **15%** threshold, with worst rows still negative. The dilated raw-CFA gate modestly improves Z8 but does not beat the X2D baseline, and the ISO 200 noise-clean sweep shows sensor-noise removal alone is too small to close the gap. Next work needs a stronger raw-domain target/model before the 50 MP / 100 MP still/editor-latitude gate. |
