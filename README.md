...
| **4. PSF-aware video/SR** | 4K cleanup and 8K reconstruction should account for the actual blur from Bayer resize/capture instead of guessing. | Approved 4K cleanup and 8K SR baselines, 1,024-tile detail budget, native Mission 1 high/low inventory, measurement plan, first PSF measurement run, kernel-stability audit, a hash-strict controlled-capture request, and metric-bearing Mission42/Z8 same-cell detail reruns. | Current near-time pairs are diagnostic only: 2 accepted pairs, max normalized-weight std **0.809** against a **0.10** gate, and one accepted pair with invalid negative weights. Controlled high/low pairs still need source hashes, decoded Bayer hashes, exact 8192 x 6144 / 4096 x 3072 dimensions and uint16 byte counts, extraction/settings/measurement receipt hashes, fixed settings, negative controls, and a stable native kernel before a PSF-conditioned model can beat the current baselines. |
...
| RAW video PSF/SR improvement | **45%** | Same-cell detail metrics now exist for Mission42/Z8; formal PSF-aware replacement is still open. |
...
|---|---|
| What is done, percent-wise, across the four pillars? | [`docs/PRODUCT_PILLAR_SCORECARD.md`](docs/PRODUCT_PILLAR_SCORECARD.md) and `/Volumes/OWC_8TB/gpr_work/artifacts/product_pillar_scorecard_psf_detail_metric_audit_20260701/index.html` |
...
| Four-pillar product scorecard | [`docs/PRODUCT_PILLAR_SCORECARD.md`](docs/PRODUCT_PILLAR_SCORECARD.md) and `/Volumes/OWC_8TB/gpr_work/artifacts/product_pillar_scorecard_psf_detail_metric_audit_20260701/index.html` |
