...
| **Premium still/SR** | 124 runtime-safe experiment receipts, raw-CFA targets, routed specialists, promotion tooling, X2D/Z8 source-evidence audits, rejected clean-source/frequency-pyramid/gated-residual/masked-detail/route-conditioned noise-aware receipts, Gate 10 source/degradation decision, failed Gate 11 route-isolated smoke, route-specialist readiness evidence, routed rendered EV-stress proxy evidence, 4/4 non-oracle editor/latitude coverage, and exact-sidecar clean-target policy exist. | Current candidates are not promotable; next evidence is `premium_still_sr_measured_degradation_teacher_source_audit_<date>`, followed only by a Gate 12 candidate intake, paired smoke, the 15% / 15% held-out recovery floor, and production submission validation. |
