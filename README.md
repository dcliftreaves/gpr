...
| **Premium still/SR** | **60%** | Raw-CFA targets, routed specialists, model-promotion tooling, the 124-receipt experiment scoreboard, X2D/Z8 source-evidence audits, rejected clean-source/frequency-pyramid/gated-residual/masked-detail/raw-CFA source-frequency/raw-CFA residual-signal/candidate-HF no-op smoke receipts, a target/degradation blocker receipt, a replacement target/source contract, a route-specialist readiness audit, routed rendered EV-stress proxy review, and 4/4 editor/latitude coverage now exist. | Route-specialist full-frame metrics are positive. Editable DNG/GPR openability and non-oracle raw-editor latitude are proven for Mission 1 DNG, Mission 1 GPR, Z8 DNG, and X2D DNG. The exact-sidecar-only noise policy passes, but no model receipt clears the 15% / 15% promotion floor or production submission validation. Next work is a replacement-contract candidate preflight that beats the paired X2D/Z8 smoke gate. |
