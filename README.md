...
| **3. Spend-time-for-quality still/SR** | Matched 1x CNN improves compressed still latitude; current 4K cleanup and 8K SR infrastructure proves the offline CNN path can emit editable raw, `.gvid`, DNG/GPR, and ProRes review artifacts; routed X2D, Z8, and Mission 1 premium still-SR specialists now have hashed fixtures, full-frame receipts, rendered proxy review, X2D 100MP editor-openability plus metadata transplant, an X2D rawpy latitude dashboard, structured HF residual targets, a blocker audit, and an executed target expansion: **13 scenes / 351 rows** across X2D and Z8 with validated noise sidecars. Raw-CFA target/trainer plumbing now has real one-scene receipts, and the raw-CFA gated probe beats the matched RGB ablation on that smoke holdout. | Real "amazing still" promotion is still open. The expanded target set proves the data loop, but the weighted render-context model was unstable and the conservative w64 control landed near zero holdout recovery. The new raw-CFA gated direction must now scale to the expanded 50 MP / 100 MP target set and pass the still/editor-latitude gate before promotion. |
