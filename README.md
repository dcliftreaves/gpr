...
| **3. Spend-time-for-quality still/SR** | Matched 1x CNN improves compressed still latitude; current 4K cleanup and 8K SR infrastructure proves the offline CNN path can emit editable raw, `.gvid`, DNG/GPR, and ProRes review artifacts; routed X2D, Z8, and Mission 1 premium still-SR specialists now have hashed fixtures, full-frame receipts, rendered proxy review, X2D 100MP editor-openability plus metadata transplant, an X2D rawpy latitude dashboard, structured HF residual targets, a blocker audit, and an executed target expansion: **13 scenes / 351 rows** across X2D and Z8 with validated noise sidecars. Raw-CFA target coverage is now complete across that expanded target, the raw-CFA gated model beats matched RGB ablations on expanded Z8 and X2D scene holdouts, a matched dilated raw-CFA gate has been tested, calibrated noise-cleaning is bounded, and a raw-CFA residual audit now shows the next target should be true source-minus-candidate same-color raw residuals. | Real "amazing still" promotion is still open. The raw-CFA gated model improves the direction but remains far below promotion: best broad holdout is **2.92%** median MAE recovery against a **15%** threshold, with worst rows still negative. The dilated raw-CFA gate modestly improves Z8 but does not beat the X2D baseline, and the ISO 200 noise-clean sweep shows sensor-noise removal alone is too small to close the gap. The raw-CFA residual audit is more promising: rendered HF residuals correlate with same-color raw residuals at **0.691** median abs corr and **0.922** median best-phase corr, so the next model should train directly on raw residuals before the 50 MP / 100 MP still/editor-latitude gate. |
| **4. RAW video improvement / PSF-aware resize** | Mission native12 4K cleanup is approved for offline review; candidate-aware 8K SR passes broad Mission42 and Z8 full-frame gates and has 8K `.gvid` plus ProRes receipts; real Mission/Z8/X2D pair-derived resize evidence confirms the current same-color 2x Bayer target is a 2x2 box model. The refreshed 1,024-tile detail budget shows the 4K-to-8K residual is essentially all same-cell fine detail, not coarse blur. A native Mission 1 high/low inventory and measurement plan now exist, and the first real measurement run executed on the selected pairs. | Native capture/display PSF is measured but not accepted for model conditioning yet: 2 of 3 pairs passed scene/alignment vetting, tile support was strong, but the fitted kernel was unstable. Next work needs controlled same-scene high/low pairs, then a PSF-conditioned SR gate before replacing the current baseline. |
...
| Premium still-SR routed specialists | X2D 100MP, Z8 50MP, and Mission 1 50MP DNG/GPR now have metadata-routed specialist checkpoints, full-frame receipts, rendered latitude proxy review, and an X2D editor-openability plus metadata-transplant receipt. Mission 1 improves full frames by **56.62% RMSE**, Z8 by **40.74%**, and X2D by **1.03%**; rendered proxy improves 33/36 crop/EV rows, with the 3 misses isolated to the X2D center crop. The X2D-scale DNG opens with transplanted source-camera metadata; q3 GPR readback is **57.49 dB** across the X2D black-to-white range. Rawpy latitude review shows median LF Y MAE **0.00546** and median HF correlation only **0.406**; source-HF oracle drops worst +2 EV MAE from **0.0916** to **0.0159**. Random-HF addback slightly worsens median MAE; the first no-REF structured HF residual model improves +2 EV holdout residual MAE by only **4.03%**. The expanded target set now covers **351 rows / 13 scenes** across X2D and Z8, with complete raw-CFA features. Naive raw-CFA channel concat was weak (**0.24%** holdout recovery versus **0.63%** RGB ablation), but the raw-CFA gated model now beats RGB on expanded holdouts: **1.04% vs 0.36%** on held-out Z8 and **2.92% vs 2.42%** on held-out X2D. The matched dilated raw-CFA gate improves Z8 to **1.30%**, but trails X2D at **2.86%** and has worse worst rows. The calibrated noise-clean sweep shows ISO 200 noise is far below the HF residual: gain 16 changes **11.93%** of pixels but only **0.24%** median residual energy. The raw-CFA residual audit is the stronger next signal: rendered HF residuals align with source-minus-candidate same-color raw residuals at **0.691** median abs corr and **0.922** median best-phase corr. That is architecture/target evidence, not a production still-SR claim. |
