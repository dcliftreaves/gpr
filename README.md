...
2. Continue Gate20 from the rejected Gate17/Gate18/Gate19/scalar/candidate-HF family: use the authorized **594 50 MP / 999 100 MP** rebuilt-supervision package, then run no-REF preflight, training, and broad audit with runtime inputs from `candidate_raw`, no REF/source/JPEG content, seconds/frame, and peak RSS.
...
Gate17 now provides that package: **1,152** balanced target rows, split into
...
Gate20 supervision/objective revision is now the active path, and the coverage blocker is closed. The expanded X2D manifest and strict target rebuild produce
**1,593** rows: **594** 50 MP rows and **999** 100 MP rows. The target coverage audit records `gate20_training_authorized=true`. The next work is Gate20 no-REF preflight, training, broad target-row audit, and full promotion receipt; not another target-construction pass, Gate17 rerun, scalar tuning, candidate-HF scaling, or safety-only no-op gate.
