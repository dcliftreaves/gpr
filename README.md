...
- **`docs/REGRESSION_2026-05-25.md`** — read this BEFORE the other 2026-05-25
  docs. Walks back the cranked-quant numbers measured on the broken multi-
  level path. Documents the 10 dB visual regression that bayer-PSNR hid.
- `docs/AUTONOMOUS_RUN_2026-05-25.md` — morning session summary
  (annotated with walk-back notes after the regression was discovered)
- `docs/SHIP_DECISION.md` — three options for shipping cranked-quant
  (annotated, the savings numbers need re-measurement on single-level)
- `docs/SPEC.md` — formal bitstream format specification (the OEM-
  contributable artifact); needs a multi-level decoder profile after fix
- `docs/methodology_cnn_aware_quant.md` — AccelIR-style methodology
  (annotated, the empirical figures need re-validation on single-level)
- `docs/quant_calibration_findings.md` — rate-distortion empirical data
  (measured on multi-level; needs re-validation)
