...
The Apache-2.0 patent grant covers GoPro's own VC-5 / CineForm / GPR
patents. It does **not** grant patent rights from third parties. The
codec inherits Green Average Subtraction (GAS) pre-processing from
upstream CineForm/GPR which arguably reads on RED `'384` (priority 2013,
expiry Feb 2034); operating it for in-camera raw video at 2K+/23+ fps
additionally falls within RED `'967` (priority 2007, expiry Apr 2028).

These are landscape sketches, not legal conclusions.
...
| ext       | what it is |
|---        |---|
| **RAW**   | Bayer sensor data, no metadata |
| **DNG**   | Adobe's open raw container |
| **GPR**   | DNG container + VC-5 compressed Bayer (stills) |
| **GVID**  | Self-describing video container (clip header + per-frame headers wrapping a sequence of VC-5 bitstreams) |
| **VC5**   | Raw VC-5 essence, no container |
| **PPM / JPG** | Uncompressed RGB and lossy preview outputs from `gpr_tools` |
...
Lightroom are trademarks of Adobe Inc. Apple Silicon is a trademark of
Apple Inc.
