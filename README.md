...
| **Premium still/SR** | **60%** | Raw-CFA targets, routed specialists, editor-openability, model-promotion tooling, the 99-receipt experiment scoreboard, X2D/Z8 source-evidence audits, rejected clean-source split smoke receipts, a route-specialist readiness audit, and routed rendered EV-stress proxy review exist. | Route-specialist full-frame metrics are positive, and rendered proxy coverage is positive, but production still needs true raw-editor latitude/openability, exact-sidecar-only noise policy, and production submission validation. |
...
true raw-editor latitude/openability receipts, exact-sidecar-only noise policy,
and production submission validation. The rendered EV-stress proxy is already
indexed separately and is not the same as raw-editor openability.
...
| **Premium still/SR** | 99 runtime-safe experiment receipts, raw-CFA targets, routed specialists, promotion tooling, X2D/Z8 source-evidence audits, rejected clean-source split receipts, route-specialist readiness evidence, and routed rendered EV-stress proxy evidence exist. | Current candidates are not promotable; next evidence is true raw-editor latitude/openability, exact-sidecar-only noise policy, and production submission validation. |
