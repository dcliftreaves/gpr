    ...
    "premium_still_sr": {
        ...
        "refs": {
            "dashboards": {
                ...
                "premium_still_sr_gate17_replacement_targets_20260702",
                "premium_still_sr_gate17_balanced_target_row_audit_20260702",
                "premium_still_sr_gate18_candidate_objective_revision_20260703",
                "premium_still_sr_gate18_tail_safe_context_target_row_audit_20260703",
...
def require_dashboard_contract(
    ...
    if entry_id == "premium_still_sr_experiment_scoreboard_teacher_first_smoke_20260702":
        ...
        if not isinstance(metrics, dict):
            ...
            try:
                ...
                if abs(t64_x2d - 0.012555669788668303) > 1e-9:
                    failures.append(f"{entry_id}: t64 Restormer X2D smoke MAE recovery drifted")
                if abs(t64_z8 - (-0.07159335574486311)) > 1e-9:
    ...
    if entry_id == "premium_still_sr_candidate_preflight_source_evidence_split_20260702":
        ...
        hashes = entry.get("hashes")
        if not isinstance(hashes, dict) or hashes.get("candidate_preflight_json_sha256") != "7c2928aaa7906fba9795bf68d14ac033bbac73942396aa0fbb15899e83a2b635":
    ...
    if entry_id == "premium_still_sr_launch_packet_source_evidence_split_20260702":
        ...
        if not isinstance(hashes, dict):
            ...
            expected_hashes = {
                "candidate_preflight_json_sha256": "7c2928aaa7906fba9795bf68d14ac033bbac73942396aa0fbb15899e83a2b635",
                "preflight_audit_json_sha256": "9a7aa461d8f66d3aef76a71bc78a57b7370ab65fdd5f39c0e1dc371c5d77addc",
                "launch_packet_json_sha256": "bc344fe21552859bbad4041ea27db5e6aedea4c495d8a35a7c5fa273b72e967d",
                "launch_packet_md_sha256": "d04b24dd7ce816d1ce1f48d40c29e39065a450969378e167c3fd53350bb81367",
    ...
    if entry_id == "premium_still_sr_source_evidence_split_teacher_smoke_20260702_next":
        if entry.get("status") != "experimental-blocker":
            failures.append(f"{entry_id}: source-evidence split smoke result must remain an experimental blocker")
        hashes = entry.get("hashes")
        if not isinstance(hashes, dict):
            failures.append(f"{entry_id}: source-evidence split smoke result needs hashes")
        else:
            expected_hashes = {
                "x2d_train_receipt_json_sha256": "cc3b4226a71cea76a9ca0018dcb07b0c8c1d44f0f4d66ee02c628386fe6be033",
                "x2d_dashboard_sha256": "e9b66cb120c37a20ad3224f3d1e6ffa3bc53cd705ece14fb28703e77dd7ce792",
                "x2d_checkpoint_sha256": "56debabc3a4dd591646833c3d797715b603ead6b4604483f9614fd44a744a1ec",
                "z8_train_receipt_json_sha256": "c9bb11a03129e6536dbe65bf4c0db7475ad69fc181026b0d25493775df61e1e6",
                "z8_dashboard_sha256": "f0fc672d30ceb8a5e21feb403389b72bf18c8957860d567fd4f47fb36e0cdfd4",
                "z8_checkpoint_sha256": "ea86ec6007cc83c083eeb2a5ca9307600f674f80caec58ae1d06376f42309f56",
            }
            for key, expected in expected_hashes.items():
                if hashes.get(key) != expected:
                    failures.append(f"{entry_id}: hash {key} drifted")
        metrics = entry.get("metrics")
        if not isinstance(metrics, dict):
            failures.append(f"{entry_id}: source-evidence split smoke result needs metrics")
        else:
            expected_exact = {
                "x2d_holdout_tile_count": 576,
                "x2d_baseline_beaten_on_holdout": 1,
                "x2d_promotion_ready": 1,
                "z8_holdout_tile_count": 1536,
                "z8_baseline_beaten_on_holdout": 0,
                "z8_promotion_ready": 0,
                "paired_smoke_gate_passed": 0,
                "long_run_blocked": 1,
                "production_ready": 0,
            }
            for key, expected in expected_exact.items():
                if metrics.get(key) != expected:
                    failures.append(f"{entry_id}: metric {key} must stay {expected!r}")
            expected_float = {
                "x2d_median_mae_recovery_pct": 0.008822244790194556,
                "x2d_median_rmse_recovery_pct": 0.003090454316737371,
                "z8_median_mae_recovery_pct": -0.07358643668344858,
                "z8_median_rmse_recovery_pct": -0.01893710250797187,
            }
            for key, expected in expected_float.items():
                try:
                    value = float(metrics.get(key))
