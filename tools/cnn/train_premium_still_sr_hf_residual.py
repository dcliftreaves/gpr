...
def make_features(x: torch.Tensor, feature_mode: str, block: int) -> torch.Tensor:
    if feature_mode == "rgb":
        return x
...
def feature_channels(feature_mode: str) -> int:
    return {"rgb": 3, "rgb_hf": 6, "rgb_hf_coord": 8}[feature_mode]
...
class HfResidualTargets:
    ...
    def row_indices(self, holdout_ev: float | None) -> tuple[list[int], list[int]]:
    ...
    def sample_batch(self, indices: list[int], batch_size: int, patch_size: int, rng: random.Random) -> tuple[torch.Tensor, torch.Tensor]:
        xs: list[np.ndarray] = []
        ys: list[np.ndarray] = []
    ...
    for idx in indices:
        ...
        for y0 in range(0, height, tile):
            for x0 in range(0, width, tile):
                ...
                pred[:, :, y0 : y0 + tile, x0 : x0 + tile] = model(make_features(cand_tile, feature_mode, feature_block))
        base_err = target
...
def write_panel_sheet(
    ...
    *,
    ...
    for row_i, idx in enumerate(selected):
        cand = torch.from_numpy(data.inputs[idx].transpose(2, 0, 1)).unsqueeze(0).to(device)
        pred = model(make_features(cand, feature_mode, feature_block)).squeeze(0).cpu().numpy().transpose(1, 2, 0)
...
<p><b>Policy:</b> trained from source-derived HF residual targets, but inference inputs are candidate RGB, candidate HF, and deterministic coordinates only.</p>
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    train_indices, holdout_indices = data.row_indices(args.holdout_ev)
    ...
    for step in range(1, args.steps + 1):
        x, y = data.sample_batch(train_indices, args.batch_size, args.patch_size, rng)
        x = x.to(DEVICE)
        y = y.to(DEVICE)
        pred = model(make_features(x, args.feature_mode, args.feature_block))
        loss = F.l1_loss(pred, y) + args.grad_weight * gradient_l1(pred, y)
    ...
    receipt = {
        ...
        "policy": {
            ...
            "runtime_inputs": "candidate_render_rgb + candidate_highpass + deterministic_xy",
...
def main() -> int:
    ...
    ap.add_argument("--feature-mode", choices=("rgb", "rgb_hf", "rgb_hf_coord"), default="rgb_hf_coord")
