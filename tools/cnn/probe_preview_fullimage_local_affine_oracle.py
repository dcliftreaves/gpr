#!/usr/bin/env python3
"""Probe spatially varying source-to-REF RGB affine ceilings for PREVIEW.

This diagnostic renders source and REF full images, downsamples them to fixed
field widths, fits independent RGB affine transforms on a spatial grid, and
scores manifest crops from the transformed source field. The local affine field
uses REF to fit each cell and is therefore an oracle only. It answers whether a
spatially varying color/low-frequency transform could close the full-image
PREVIEW blocker, or whether the gap still requires structure/detail modeling.
"""
...
def grid_bounds(length: int, cells: int) -> list[tuple[int, int]]:
    edges = np.linspace(0, length, cells + 1).round().astype(int).tolist()
    return [(edges[i], max(edges[i] + 1, edges[i + 1])) for i in range(cells)]


def fit_local_affine_field(
    source: np.ndarray,
    target: np.ndarray,
    grid: int,
    ...
    cell_stats: list[dict[str, float]] = []
    y_bounds = grid_bounds(source.shape[0], grid)
    x_bounds = grid_bounds(source.shape[1], grid)
    for y0, y1 in y_bounds:
        for x0, x1 in x_bounds:
            src_cell = source[y0:y1, x0:x1]
            tgt_cell = target[y0:y1, x0:x1]
            coef, stats = fit_rgb_affine(src_cell, tgt_cell, max_samples_per_cell)
            out[y0:y1, x0:x1] = apply_rgb_affine(src_cell, coef)
            cell_stats.append(stats)
    return out, {
        "local_grid": float(grid),
        "cell_count": float(len(cell_stats)),
        "fit_rmse_median": float(np.median([row["fit_rmse"] for row in cell_stats])),
        "fit_rmse_worst": float(max(row["fit_rmse"] for row in cell_stats)),
        "fit_mae_median": float(np.median([row["fit_mae"] for row in cell_stats])),
        "fit_mae_worst": float(max(row["fit_mae"] for row in cell_stats)),
        "fit_samples_total": float(sum(row["fit_samples"] for row in cell_stats)),
    }
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    work = Path(tempfile.mkdtemp(prefix="preview_local_affine_oracle_", dir=args.tmp_dir))
    rows: list[dict[str, Any]] = []
    ...
    try:
        for image in images:
            ...
            print(f"[local-affine-oracle] render {image_id}", flush=True)
            ...
            for max_width in args.max_width:
                print(f"[local-affine-oracle] {image_id} max_width={max_width}", flush=True)
                ...
                ref_low_arr = np.asarray(ref_low, dtype=np.uint8)
                for grid in args.grid:
                    local_arr, fit = fit_local_affine_field(
                        source_low_arr,
                        ref_low_arr,
                        grid,
                        args.max_fit_samples_per_cell,
                    )
                    local_field = Image.fromarray(local_arr)
                    for crop_name, crop in crops.items():
                        ref_crop = crop_metric_image(ref, crop, image["sensor_dims"])
                        source_crop = crop_metric_image(source, crop, image["sensor_dims"])
                        local_crop = crop_metric_image(local_field, crop, image["sensor_dims"])
                        score_crop(
                            args=args,
                            rows=image_rows,
                            image_id=image_id,
                            crop_name=crop_name,
                            variant=f"local_affine_grid{grid}_w{max_width}",
                            max_width=max_width,
                            ref_crop=ref_crop,
                            out_crop=local_crop,
                            fit=fit,
                        ...
                        for sigma in args.high_sigma:
                            score_crop(
                                ...
                                image_id=image_id,
                                crop_name=crop_name,
                                variant=f"local_affine_grid{grid}_source_high_s{sigma:g}_w{max_width}",
                                max_width=max_width,
                                ref_crop=ref_crop,
                                out_crop=source_high_composite(local_crop, source_crop, sigma),
                                fit=fit,
                            )
                    local_field.close()
                source_low.close()
    ...
    return {
        "schema": "preview_fullimage_local_affine_oracle.v1",
        ...
        "grids": [int(v) for v in args.grid],
        "high_sigma": [float(v) for v in args.high_sigma],
        "oracle_variants_not_allowed_for_production": ["local_affine_grid*_w*", "local_affine_grid*_source_high_s*_w*"],
        "render_contract": {
            "oracle_uses_ref_to_fit_local_affine": True,
            "production_allowed": False,
            "question": "whether spatially varying RGB affine closes full-image PREVIEW source/REF mismatch",
        },
        "timing": {
...
"""
    ...
    parts = [
        "<!doctype html><meta charset='utf-8'><title>PREVIEW local affine oracle</title>",
        f"<style>{css}</style><h1>PREVIEW Full-Image Local Affine Oracle</h1>",
        "<p>Local affine variants fit source to REF per spatial grid cell and are diagnostic only.</p>",
        "<div class=cards>",
...
def main() -> int:
    ...
    ap.add_argument("--grid", type=int, action="append", default=None)
    ap.add_argument("--high-sigma", type=float, action="append", default=None)
    ap.add_argument("--max-fit-samples-per-cell", type=int, default=25_000)
    ...
    for row in payload["summary"]:
        print(
            f"{row['variant']:<56} {row['pass_count']:>3}/{row['count']:<3} "
