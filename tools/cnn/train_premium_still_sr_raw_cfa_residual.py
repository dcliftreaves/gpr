...
def eval_rows(
    ...
    *,
    ...
    for idx in indices:
        ...
        target = torch.from_numpy(data.target[idx].transpose(2, 0, 1)).unsqueeze(0).to(device)
        ...
        for y0 in range(0, height, tile):
            for x0 in range(0, width, tile):
                ...
                pred[:, :, y0 : y0 + tile, x0 : x0 + tile] = model(make_features(raw_tile, feature_mode, feature_block, ev, noise))
        base_err = target
...
def write_panel_sheet(
    ...
    *,
    ...
    for row_i, idx in enumerate(selected):
        ...
        ev = torch.tensor([float(data.rows[idx].get("ev", 0.0))], dtype=torch.float32, device=device)
        noise = torch.tensor([data.noise_features[idx]], dtype=torch.float32, device=device)
        pred = model(make_features(raw, feature_mode, feature_block, ev, noise)).squeeze(0).cpu().numpy().transpose(1, 2, 0)
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    train_eval = eval_rows(model, data, train_indices, feature_mode=args.feature_mode, feature_block=args.feature_block, device=device, tile=args.eval_tile)
    holdout_eval = None
    if holdout_indices:
...
def main() -> int:
    ...
    ap.add_argument("--feature-mode", choices=("raw", "raw_hf", "raw_hf_coord_ev_noise", "raw_multiscale_coord_ev_noise"), default="raw_multiscale_coord_ev_noise")
