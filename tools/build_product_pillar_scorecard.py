...
def build_scorecard(external_root: Path) -> dict[str, Any]:
    pillars = [
        ...
        },
        {
            ...
            "open_work": [
                "Follow the next-experiment contract: train against the deduplicated raw target and replace the weak first raw-CFA residual learner with a stronger CFA-aware teacher/objective that clears both Z8 and X2D held-out raw-residual gates; the current blocker is X2D raw-detail recovery strength plus camera-dependent target SNR and target-energy/domain shift, not stored-HF features, naive noise subtraction, local loss-weight tuning, simple pooled-context feature concatenation, combined local-feature concatenation, multiscale band-loss reweighting, camera-domain filtering, camera-balanced sampling, 32px context padding, scalar target-energy row weighting, a small U-Net alone, frame-context scalar planes alone, bounded full-crop sampling alone, bounded full-crop stored-HF/context U-Net training, bounded full-crop spectral-loss U-Net training, larger full-crop raw-context U-Net training, deeper gated pyramid U-Net training, bounded global-context U-Net training, RCAB-only scale-up, simple NAF-style scale-up, or nearest-neighbor residual patch retrieval over current candidate features.",
    ...
    return {
        ...
        "next_actions": [
            ...
            "Continue premium still-SR from the current raw-CFA residual blocker: held-out Z8 is mildly positive, hard X2D is only barely positive after wider context, and stored-HF/noise-threshold/pooled-context/combined-context/band-loss/X2D-only-domain/target-energy weighting probes did not solve it.",
