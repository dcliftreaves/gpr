    ...
    pillars = [
        ...
        },
        {
            ...
            "done_evidence": [
                ...
                "Two first PSF-conditioned X2D scene-holdout controls now exist. A local noise-floor U-Net with near-box PSF scalar planes reaches about 0.106 percent median exact raw MAE recovery versus the non-PSF 0.153 percent baseline, while a full-crop raw-context PSF U-Net reaches about 0.064 percent. The PSF metadata gap audit then confirms 0 of 117 deduplicated target rows have row-level PSF metadata, so the current blocker is not missing PSF plumbing; it is missing per-row/per-camera PSF variation or a stronger teacher/objective.",
