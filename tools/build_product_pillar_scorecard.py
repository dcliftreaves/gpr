    ...
    pillars = [
        ...
        },
        {
            ...
            "done_evidence": [
                ...
                "A bounded NAF-like residual pixelshuffle plus gradient/detail-loss probe is also rejected on the routed clean-source pairs: the 500-step X2D holdout is about -0.35 percent median MAE and -0.36 percent RMSE versus interpolation, and the matched Z8 holdout regresses by about -10.09 percent median MAE.",
                "A premium still-SR noise-policy gate now machine-checks the current clean-signal and model receipts. It confirms the clean target has 117/117 calibrated noise sidecars and forbids source/REF/JPEG/exact-source-noise runtime use, while keeping all supplied still-SR model receipts blocked because none clear the 15 percent holdout MAE/RMSE floors.",
                ...
                "The refreshed premium still-SR experiment scoreboard now scans 95 rendered-HF, raw-CFA residual, clean-signal, and clean-source pair training receipts. It records 95 runtime-safe rows, zero promotable rows, and a best older runtime-safe holdout of 4.03 percent MAE / 3.75 percent RMSE recovery, far below the 15 percent / 15 percent promotion threshold. The newest t64 Restormer clean-source pair receipts also fail: X2D smoke is only about +0.013 percent median MAE recovery, Z8 smoke is about -0.072 percent, the longer Z8 pass overfits train while regressing held-out MAE to about -5.06 percent, and the degradation/objective ablation with Charbonnier, Laplacian, RAW noise, gain jitter, and blur reaches only +0.0048 percent X2D MAE recovery while regressing Z8 by about -0.397 percent.",
            ],
            "open_work": [
                ...
                "Use calibrated noise sidecars as conditioning, then add back only noise proven separate from signal; keep the premium still-SR noise-policy gate passing before any promotion claim.",
            ],
            "evidence": annotate_refs(
                [
                    ...
                    artifact_ref("premium still-SR experiment scoreboard", "artifacts/premium_still_sr_experiment_scoreboard_restormer_degrade_t64_20260702/index.html"),
                    ...
                    artifact_ref("premium still-SR routed NAF/detail X2D rejection", "artifacts/premium_still_sr_clean_source_pair_model_routed_x2dholdout_naf_grad_w48_500_20260702/index.html"),
                    artifact_ref("premium still-SR routed NAF/detail Z8 rejection", "artifacts/premium_still_sr_clean_source_pair_model_routed_z8holdout_naf_grad_w48_500_20260702/index.html"),
                    artifact_ref("premium still-SR noise-policy gate", "artifacts/premium_still_sr_noise_policy_gate_20260702/index.html"),
