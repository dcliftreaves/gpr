...
|---|---|
| Firmware capture integration | sensor/DMA handoff and memory ownership have not been executed on target; current receipts use Pi stand-in file input. `tools/mission1_dma_source_sim.py` now provides a deterministic separate-process FIFO source simulator for replaying measured Mission 1 cadence/backpressure on the Pi, but its receipts are explicitly non-production evidence. |
