...
| PSF-aware RAW video improvement | 45% | Current 4K cleanup and 8K SR baselines are useful, including continuous 8K no-CNN versus CNN ProRes review media for a whole-scene A/B; near-time native Mission 1 high/low candidates are indexed, the first native PSF measurement has executed, the kernel-stability audit identifies coefficient disagreement, a hash-strict capture request now spells out the controlled-pair capture and model-gate path, and the current Mission42/Z8 baseline/candidate summaries now expose same-cell Bayer fine-detail metrics. Formal native PSF/blur-aware replacement remains open because the available near-time pairs produce an unstable kernel and no PSF-conditioned model has beaten the approved baseline under those same-cell detail metrics plus the existing worst-row gates. |
