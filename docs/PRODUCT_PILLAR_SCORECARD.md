...
| Premium still/SR | 60% | The expanded 13-scene / 351-row target set now has complete raw-CFA features, the deduplicated raw-supervision NPZ collapses it to 117 unique scene/crop raw-domain rows with zero raw conflicts, and RCAB/NAF/U-Net teacher receipts run on that target. Z8 is mildly positive, but X2D remains far below promotion: the RCAB smoke is only **0.069%** median recovery on an 8-row X2D holdout, the scaled RCAB pass is only **0.034%** on a 24-row X2D holdout, the all-X2D-holdout NAF-style pass is **-0.059%** on a 24-row X2D holdout with a heavily regressed train split, the corrected X2D-scene NAF pass reaches only **0.107%** median MAE recovery and negative RMSE recovery, and the matched X2D-scene U-Net batch shows hard SNR filtering hurts: signal-only **0.112%**, signal-or-mixed **0.119%**, unfiltered X2D-only **0.149%**. same-scene candidate-signal and frequency-filter probes regress, nearest-neighbor retrieval regresses the hard X2D holdout, and candidate-only local/full-crop/global-context/masked-context statistics remain insufficient. The raw-target SNR audit shows why one unweighted residual objective is wrong, but binary row removal is not the fix; the next pass needs a materially stronger CFA-aware teacher/data objective with noise-aware weighting, camera conditioning, PSF conditioning, and spatial + Fourier losses. |
