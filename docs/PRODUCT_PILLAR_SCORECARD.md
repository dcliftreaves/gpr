...
`/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_experiment_scoreboard_window_attention_20260702/index.html`.
It now ranks 99 runtime-safe rendered-HF, raw-CFA residual, clean-signal, and
...
threshold, while the newest launchable window-attention clean-source smoke is
also rejected: X2D is +0.0016 percent median MAE with -0.0095 percent RMSE, and
Z8 is -0.5807 percent median MAE with -0.3829 percent RMSE. Restormer,
degradation/objective ablations, teacher-first, and window-attention branches
are diagnostic rather than production-ready.
...
The current premium still-SR source-evidence audits live at
`/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_source_evidence_x2dholdout_t64_20260702/index.html`
and
`/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_source_evidence_z8holdout_t64_20260702/index.html`.
They run a candidate-only local-ridge probe against nearest same-color Bayer
2x. X2D passes the 1 percent source-evidence floor at 4.821 percent median MAE
and 11.520 percent median RMSE recovery; Z8 remains blocked at 0.650 percent
median MAE recovery even though RMSE improves by 21.900 percent. That makes the
