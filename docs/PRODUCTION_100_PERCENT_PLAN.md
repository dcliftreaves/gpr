...
| 20 | open | Build `premium_still_sr_gate14_candidate_intake_<date>`: persist the executable multi-source selector sidecar, source-model mapping, feature schema, hashes, and no-op fallback; prove it reproduces the Gate 13 upper-bound pass from candidate-only runtime inputs. | Gate 14 executable selector intake has every X2D image median MAE `> 0.001%`, global and per-image worst-row MAE `>= 0%`, no REF/source/JPEG/gate metrics as runtime inputs, and Z8 exact-noop remains `0.0%` / `0.0%`. |
| 21 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 22 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 23 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 24 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
|---|---|---|---|---|
| A: Premium still-SR Gate 14 | open, local | Continue from `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_gate13_source_or_objective_revision_20260702/index.html`. Build an executable multi-source selector intake from the passing upper bound before any long train. | `premium_still_sr_gate14_candidate_intake_<date>` | The selector sidecar, source-model mapping, feature schema, hashes, and exact no-op fallback reproduce the Gate 13 X2D per-image pass from candidate-only runtime inputs, with Z8 exact-noop preserved. |
| A2: Premium still-SR promotion | open after Gate 14 | After Gate 14 passes, produce a no-REF candidate-only model receipt that clears the 15% / 15% held-out floor, then run `check_premium_still_sr_promotion_gate.py` and `check_production_capture_submission.py`. | `premium_still_sr_promotion_receipts` | Full 50 MP / 100 MP routed gates pass, worst-row recovery is nonnegative, raw-editor latitude/openability opens for every route, timing/memory/checkpoint hashes exist, exact-sidecar-only noise policy passes, and `check_production_capture_submission.py` passes. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate 10 classifies the blocker as `source_degradation_target_mismatch`: X2D target distribution is `3.4500243590744026x` the same-camera train median and Z8 is mostly noise-floor targets (`28/36`). Gate 11 route-isolated teacher/router smoke fails, Gate 12 paired smoke fails, and single-source Gate 13 gating fails per scene. Gate 13 source/objective revision now passes as a multi-source selector upper bound: 12 compatible X2D sources, 10,199 safe source/predicate selectors, `x2d_2025_austin_06` median/worst MAE `8.022846730221168%` / `0.0%`, `x2d_2025_austin_07` median/worst MAE `0.07380457072746566%` / `0.0%`, and Z8 exact-noop `0.0%` / `0.0%`. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from rejected single-source branches. The next local action is `premium_still_sr_gate14_candidate_intake_<date>`: make the multi-source selector executable, persist sidecars/hashes/source mapping, and prove it reproduces the Gate 13 pass from candidate-only runtime inputs before long training. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
