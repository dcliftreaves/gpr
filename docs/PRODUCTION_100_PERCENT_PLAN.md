...
| 7 | open | Build a Premium Still/SR target/degradation evidence receipt that fixes the raw-CFA objective failure mode. Frequency-pyramid, gated no-op residual, masked-detail/no-op, raw-CFA source-frequency, raw-CFA residual signal, and candidate-HF no-op-gated candidates are all blocked before long training by checked receipts; do not tune their thresholds or rerun them as production work. The next candidate must provide new target/degradation evidence or a materially different route-conditioning receipt, then pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass` on both X2D and Z8 before any long run. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
