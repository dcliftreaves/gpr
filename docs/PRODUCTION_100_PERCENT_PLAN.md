...
| 5 | in progress | The editor/latitude coverage audit now proves X2D 100 MP DNG with non-oracle rawpy/LibRaw latitude and editable DNG/GPR openability. Next local commands are to package Mission 1 DNG, Mission 1 GPR, and Z8 DNG routed candidates, build matching editor/openability and latitude receipts, then rerun the coverage audit. | `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_editor_latitude_coverage_20260702/coverage.json` must reach 4/4 ready routes. |
| 6 | open | Wire exact-sidecar-only noise policy into every routed Premium Still/SR target after editor/latitude coverage is 4/4. | Premium still-SR noise policy receipt with no source residual, no REF/source/JPEG content, and exact camera/ISO sidecar usage only. |
| 7 | open | Build and validate the Premium Still/SR production submission. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
| 8 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 9 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 10 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 11 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
|---|---|---|---|---|
| A: Premium still-SR promotion | open, local | Continue from `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_editor_latitude_coverage_20260702/index.html`. X2D 100 MP DNG is ready for the raw-editor/openability sub-gate; Mission 1 50 MP DNG, Mission 1 50 MP GPR, and Z8 50 MP DNG are not. Next command class: package those three routed candidates into editable DNG/GPR with metadata transplant, run non-oracle rawpy/LibRaw latitude review, rerun the coverage audit, then wire exact-sidecar-only noise policy and run `check_production_capture_submission.py`. | `premium_still_sr_promotion_receipts` | Full 50 MP / 100 MP routed gates pass, worst-row recovery is nonnegative, raw-editor latitude/openability opens for every route, timing/memory/checkpoint hashes exist, exact-sidecar-only noise policy passes, and `check_production_capture_submission.py` passes. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 99 runtime-safe receipts and 0 promotable receipts. The clean-source split smoke receipts narrow one rejected path: X2D passes the short gate at `+0.008822244790194556%` median MAE and `+0.003090454316737371%` median RMSE recovery, while Z8 fails at `-0.07358643668344858%` median MAE and `-0.01893710250797187%` median RMSE. The route-specialist readiness audit now identifies the next production direction: Mission 1 50 MP DNG/GPR, Z8 50 MP DNG, and X2D 100 MP DNG have route coverage and positive full-frame metric floors. The routed rendered EV-stress proxy is also linked and ready: 36 rows, Mission1/Z8/X2D coverage, and 33 better versus 3 worse rows. The editor/latitude coverage audit is now 1/4 ready: X2D 100 MP DNG passes non-oracle raw-editor latitude and editable DNG/GPR openability; Mission 1 DNG/GPR and Z8 DNG remain missing. | Do not start a long run from the source-evidence split candidate. Continue the route-specialist/raw-CFA path by packaging Mission 1 DNG/GPR and Z8 DNG routed candidates into editable raw receipts, running non-oracle raw-editor latitude reviews, wiring exact-sidecar-only noise policy, and building the production submission. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
