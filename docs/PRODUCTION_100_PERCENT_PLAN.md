...
| 16 | open | Build `premium_still_sr_gate13_degradation_source_upgrade_<date>`: audit/construct a stronger X2D degradation/teacher source that changes the training target enough to beat nearest same-color without REF/JPEG/source content at runtime. | A preflight source receipt identifies the blocker as fixed only if X2D source/teacher smoke has median MAE `> 0.001%`, worst-row MAE `>= 0%`, and `baseline_beaten_on_holdout=true`; otherwise it classifies the next blocker as source/degradation mismatch, model capacity, objective/gating failure, or insufficient clean source. |
| 17 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 18 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 19 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 20 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate 10 classifies the blocker as `source_degradation_target_mismatch`: X2D target distribution is `3.4500243590744026x` the same-camera train median and Z8 is mostly noise-floor targets (`28/36`). Gate 11 route-isolated teacher/router smoke fails: X2D median/worst raw MAE recovery is `-0.09995100006746782%` / `-2.156844783012532%`, and Z8 is `0.0%` / `-14.118886237720433%`. Gate 12 paired smoke also fails: X2D median/worst MAE recovery is `-0.015976613123677263%` / `-0.22449340376395477%`, while Z8 exact-noop passes. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from the rejected source-evidence split, frequency-pyramid, gated-residual, masked-detail/no-op, source-frequency, residual-signal, candidate-HF no-op, simple frame-context, Gate 9 route-conditioned/noise-aware, Gate 11 route-isolated residual, or Gate 12 synthetic teacher smoke candidates. The next local action is `premium_still_sr_gate13_degradation_source_upgrade_<date>`: construct/audit a stronger X2D degradation/teacher source that beats nearest same-color before long training. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
