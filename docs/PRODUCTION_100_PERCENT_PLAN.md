...
| 7 | open | Build a no-op/benefit-gated residual Premium Still/SR candidate. The latest frequency-pyramid source-evidence branch is blocked before long training by `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_frequency_pyramid_smoke_gate_acceptance_20260702/smoke_gate_acceptance.json`: X2D fails worst-row MAE and Z8 regresses median/worst-row MAE. The next candidate must pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass` before any long run. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 113 runtime-safe receipts and 0 promotable receipts. The clean-source split smoke receipts narrow one rejected path: X2D passes the short gate at `+0.008822244790194556%` median MAE and `+0.003090454316737371%` median RMSE recovery, while Z8 fails at `-0.07358643668344858%` median MAE and `-0.01893710250797187%` median RMSE. The newer frequency-pyramid source-evidence branch is also blocked before long training: X2D median MAE is only `+0.005422090158202289%` and worst-row MAE is `-4.850145322879209%`; Z8 median MAE is `-8.809287941837436%` and worst-row MAE is `-67.44360239254922%`. The route-specialist readiness audit identifies the production route surface: Mission 1 50 MP DNG/GPR, Z8 50 MP DNG, and X2D 100 MP DNG have route coverage and positive full-frame metric floors. The routed rendered EV-stress proxy is linked and ready: 36 rows, Mission1/Z8/X2D coverage, and 33 better versus 3 worse rows. The editor/latitude coverage audit is 4/4 ready. The clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from the rejected source-evidence split or frequency-pyramid candidates. The next local action is a no-op/benefit-gated residual model candidate whose paired X2D/Z8 smoke receipts pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass`, then a production submission only if the 15% / 15% floor is cleared. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
