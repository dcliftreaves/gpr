...
| 8 | open | Build a replacement Premium Still/SR target/degradation source receipt or a materially different route-conditioning candidate. Frequency-pyramid, gated no-op residual, masked-detail/no-op, raw-CFA source-frequency, raw-CFA residual signal, and candidate-HF no-op-gated candidates are all blocked before long training by checked receipts; do not tune their thresholds or rerun them as production work. The next candidate must pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass` on both X2D and Z8 before any long run. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
| 9 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 10 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 11 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 12 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
