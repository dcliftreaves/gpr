...
| 18 | open | Build `premium_still_sr_gate13_feature_rich_tail_safe_source_smoke_<date>`: use richer candidate-only runtime statistics or scene-normalized gates so every X2D image keeps positive median MAE while all negative rows no-op. | Every X2D image has median MAE `> 0.001%`, global and per-image worst-row MAE are `>= 0%`, no REF/source/JPEG/gate metrics are runtime inputs, and Z8 exact-noop stays `0.0%` / `0.0%`. |
| 19 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 20 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 21 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 22 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate 10 classifies the blocker as `source_degradation_target_mismatch`: X2D target distribution is `3.4500243590744026x` the same-camera train median and Z8 is mostly noise-floor targets (`28/36`). Gate 11 route-isolated teacher/router smoke fails: X2D median/worst raw MAE recovery is `-0.09995100006746782%` / `-2.156844783012532%`, and Z8 is `0.0%` / `-14.118886237720433%`. Gate 12 paired smoke also fails: X2D median/worst MAE recovery is `-0.015976613123677263%` / `-0.22449340376395477%`, while Z8 exact-noop passes. Gate 13 finds a positive-median X2D source (`+0.2741207579275717%`) but blocks it because worst-row MAE is `-2.959145874624423%`. The Gate 13 tail-safe source smoke then finds simple candidate-only gates are aggregate-only: best global median MAE is `+0.215125015196241%` with `0.0%` worst-row MAE, but strict per-image rule count is `0` and one X2D scene median falls to `0.0%`. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from the rejected source-evidence split, frequency-pyramid, gated-residual, masked-detail/no-op, source-frequency, residual-signal, candidate-HF no-op, simple frame-context, Gate 9 route-conditioned/noise-aware, Gate 11 route-isolated residual, Gate 12 synthetic teacher smoke, ungated Gate 13 positive-median source, or simple Gate 13 tile-stat gate. The next local action is `premium_still_sr_gate13_feature_rich_tail_safe_source_smoke_<date>`: preserve the positive X2D median per scene and make worst-row MAE nonnegative before long training. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
