...
The next command must add or locate more X2D/100 MP rebuilt-supervision sources
with validated camera-noise sidecars and rerun strict target expansion. Do not
...
| 25 | closed, coverage blocker | Gate20 target expansion, expanded target generation, strict planner rerun, and coverage audit ran. | Actual rebuild has 351 rows: 108 50 MP and 243 100 MP. Strict planner reaches 594 50 MP but only 243 100 MP; `gate20_training_authorized=false`. |
| 26 | open | Add or locate more X2D/100 MP DNG sources with validated camera-noise sidecars, then rerun strict Gate20 target expansion. | Strict target coverage records 50 MP rows >= 576, 100 MP rows >= 576, total rows >= 1152, and authorizes training. |
| 27 | open | If the rebuilt target receipt authorizes training, run Gate20 training, broad target-row audit, and then the strict promotion receipt from route readiness, editor/openability, noise policy, formal promotion gate, and production-capture requirements. | `premium_still_sr_promotion_receipts` pass the 15% / 15% held-out MAE/RMSE floor and production submission checker, or classify the exact blocker. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate16 paired smoke passed on a narrow branch, but the all-target-row audit rejects it for production promotion: `463` X2D/100 MP target-tile rows, no 50 MP rows, no full-frame scope, `-0.12226915231999792%` median MAE recovery, `-0.1296250122706981%` median RMSE recovery, and `-9.625700832601128%` worst-row MAE recovery. Gate17 supplies a balanced target package with `576` 50 MP rows and `576` 100 MP rows, candidate-only runtime policy, and no production claim. Gate17, Gate18, and Gate19 are all rejected by broad audits; Gate17 scalar-direction calibration is rejected; candidate-HF feature scaling is rejected. Gate20 target rebuilding now works but is coverage-blocked: actual coverage is 108 50 MP rows and 243 100 MP rows; strict planning can reach 594 50 MP rows but still only 243 100 MP rows. | Do not train Gate20 yet. Add or locate more X2D/100 MP DNG sources with validated sidecars, rerun strict target expansion until both classes clear 576 rows, then run no-REF preflight and train/audit only if the target coverage receipt authorizes it. | The replacement candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
