...
| 7 | open | Build a Premium Still/SR candidate that fixes the raw-CFA route failure mode. Frequency-pyramid, gated no-op residual, masked-detail/no-op, raw-CFA source-frequency, and generic raw-CFA residual signal candidates are all blocked before long training by checked receipts; do not tune their thresholds or rerun them as production work. The next candidate must add candidate-only no-op/benefit gating, route-specific Z8 residual conditioning, or new target/degradation source evidence, then pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass` on both X2D and Z8 before any long run. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
