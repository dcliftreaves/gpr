...
| 2 | Premium still-SR candidate preflight | The source-evidence split launch packet now exists at `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_launch_packet_source_evidence_split_20260702/index.html`. Run its X2D and Z8 smoke commands next. | `candidate_preflight.json`, `preflight_audit.json`, and `launch_packet.json` validate with `launchable_preflight_passed`. | Long training is allowed only after both smoke holdouts beat nearest same-color Bayer 2x with positive median MAE/RMSE and nonnegative worst-row MAE. |
| 3 | Premium still-SR promotion | Run the full 50 MP / 100 MP gate only after the smoke gate passes. | `premium_still_sr_promotion_receipts` and production submission audit. | Mark Premium still/SR 100% only if the production checker passes; otherwise record the exact blocker class. |
