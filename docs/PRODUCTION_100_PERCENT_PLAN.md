...
| 7 | open | Build a Premium Still/SR candidate with a materially different target/source or teacher objective. Frequency-pyramid, gated no-op residual, and masked-detail/no-op target-objective candidates are all blocked before long training by checked receipts; do not tune their thresholds or rerun their residual family as production work. The next candidate must pass `check_premium_still_sr_smoke_gate_acceptance.py --require-pass` on both X2D and Z8 before any long run. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
