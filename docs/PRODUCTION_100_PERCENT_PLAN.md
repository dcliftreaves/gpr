...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate 10 classifies the blocker as `source_degradation_target_mismatch`: X2D target distribution is `3.4500243590744026x` the same-camera train median and Z8 is mostly noise-floor targets (`28/36`). Gate 11 route-isolated teacher/router smoke fails, Gate 12 paired smoke fails, and single-source Gate 13 gating fails per scene. Gate 14 selector smoke now closes the sidecar/runtime-smoke step: seven rules, six source mappings, 78 runtime-safe features, X2D selector-smoke median/worst MAE `0.329828330762138%` / `0.0%` and `0.02786331921791634%` / `0.0%`, source model failure count `0`, and Z8 exact-noop `0.0%` / `0.0%`. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from rejected single-source branches. The next local action is `premium_still_sr_promotion_receipts`: run full 50 MP / 100 MP promotion validation from the Gate 14 routed selector with timing/memory, editor/openability, exact-sidecar-only noise policy, and production submission validation. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
