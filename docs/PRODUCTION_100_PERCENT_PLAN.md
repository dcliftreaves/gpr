...
| 3 | in progress | Do not launch a longer Premium Still/SR run from this candidate. Build the next candidate only after changing the Z8 source/degradation target, teacher objective, or camera conditioning, then run new paired X2D/Z8 smoke receipts. | Updated Premium Still/SR blocker/candidate receipt linked from the manifest/scorecard. |
| 4 | open | If Step 3 passes, run the full Premium Still/SR production gate and produce editable DNG/GPR, review media, timing, memory, checkpoint/config hashes, and exact-sidecar-only noise-policy receipts. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
| 5 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 6 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 7 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 8 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
| 3 | Premium still-SR promotion | Build the next candidate only after a material Z8 source/degradation repair, then rerun paired X2D/Z8 smoke gates. | `premium_still_sr_promotion_receipts` and production submission audit. | Mark Premium still/SR 100% only if the production checker passes; otherwise record the exact blocker class. |
| 4 | Raw-stills noise sidecars | Build Mission/iPhone darkframe sidecars only from strict-provenance true darkframes. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` sidecar/audit receipts. | Mark Best RAW stills 100% only after both sidecars validate with no-scene-signal provenance. |
| 5 | Mission 1 camera-role video MVP | Hand off the camera validation runbook; only a real camera-role run can close the gate. | `mission1_camera_role_receipts`. | Mark Raw video MVP 100% only after real source/storage/display receipts replace Pi stand-ins. |
| 6 | Locked raw-video reconstruction | Protect the approved 4K cleanup and 8K SR evidence. | Lock ledger, release manifest, and CI guards pass. | Do not run new video SR as production work unless a locked receipt fails. |
...
|---|---|---|---|---|
| A: Premium still-SR promotion | open, local | Build the next candidate only after changing source evidence, degradation synthesis, or teacher objective. The latest source-evidence audit shows X2D has candidate-only local signal (`4.82%` MAE / `11.52%` RMSE recovery), while Z8 does not clear the 1% MAE source-evidence floor (`0.65%` MAE / `21.90%` RMSE recovery). The next preflight must use the X2D evidence and repair the Z8 source/degradation mismatch; another architecture swap on the same pair target is rejected. Start with `python3 tools/build_premium_still_sr_candidate_preflight_template.py --output /Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_candidate_preflight_<date>/candidate_preflight.json`, then edit the manifest and run `python3 tools/build_premium_still_sr_launch_packet.py --manifest /Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_candidate_preflight_<date>/candidate_preflight.json --output-dir /Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_launch_packet_<date> --require-launchable` | `premium_still_sr_promotion_receipts` | Launch preflight passes, X2D and Z8 smoke holdouts beat same-color Bayer interpolation, full 50 MP / 100 MP gate passes, worst-row recovery is nonnegative, editor-latitude review opens, timing/memory/checkpoint hashes exist, and `check_production_capture_submission.py` passes. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 99 runtime-safe receipts and 0 promotable receipts. The rejected relaunch guard blocks `teacher_first_fullframe_raw_sr_smoke_v1`; the newer window-attention clean-source candidate passes launch preflight but fails the X2D/Z8 smoke gate before long training. X2D holdout median MAE recovery is `+0.001614994503120611%` with RMSE `-0.0094530994823499%`; Z8 holdout median MAE recovery is `-0.5807092150718575%` with RMSE `-0.3829316485837014%`. The candidate-only source-evidence audit finds X2D local signal (`4.821260781753699%` MAE / `11.520193787949786%` RMSE recovery) but flags Z8 as a blocker (`0.649807764458084%` MAE, below the 1% floor). The source-evidence split smoke receipts now narrow the blocker further: X2D passes the short gate at `+0.008822244790194556%` median MAE and `+0.003090454316737371%` median RMSE recovery, while Z8 fails at `-0.07358643668344858%` median MAE and `-0.01893710250797187%` median RMSE. | Do not start a long run from the source-evidence split candidate. Build the next candidate only after changing the Z8 source/degradation target, teacher objective, or camera conditioning, then rerun paired X2D/Z8 smoke gates. | The candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, and passes production submission validation. |
