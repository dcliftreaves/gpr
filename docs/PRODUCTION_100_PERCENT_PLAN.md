...
| 7 | open | Build or identify a no-REF Premium Still/SR model receipt that clears the 15% / 15% held-out MAE/RMSE floor, then validate the Premium Still/SR production submission. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
