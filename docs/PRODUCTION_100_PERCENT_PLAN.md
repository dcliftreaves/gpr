...
| 2 | Premium still-SR candidate preflight | Create a launch packet whose material change directly addresses the source-evidence result: X2D may use local source evidence as supervision/objective; Z8 must change source/degradation target before long training. | `candidate_preflight.json`, `audit.json`, and launch packet dashboard. | Long training is allowed only after both smoke holdouts beat nearest same-color Bayer 2x with positive median MAE/RMSE and nonnegative worst-row MAE. |
