...
| 22 | open | Run the full Premium still-SR promotion validation from the Gate 14 routed selector: 50 MP and 100 MP holdouts, nonnegative worst-row recovery, editor/openability, timing/memory, checkpoint hashes, exact-sidecar-only noise policy, and production submission validation. | `premium_still_sr_promotion_receipts` pass the 15% / 15% held-out MAE/RMSE floor and production submission checker, or classify the exact blocker. |
