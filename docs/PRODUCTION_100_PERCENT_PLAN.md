...
The first open local gate is **Gate A: Premium still-SR promotion**. Gate 14
candidate intake is now closed, so the next command must run the persisted
selector sidecar through the actual model-loading/render path or update that
selector-smoke receipt with a precise blocker. Do not start optional video SR,
PSF research, or new dashboard cosmetics while Gate A has a local next command.
...
| 21 | open | Run `premium_still_sr_gate14_selector_smoke_<date>`: execute the persisted sidecar through the actual selector/model-loading render path and compare against the intake replay. | The selector smoke reproduces the Gate 14 X2D pass, preserves Z8 exact-noop, records source checkpoint hashes, rejects feature-schema drift, uses no REF/source/JPEG/gate metric inputs, and decides whether promotion is allowed. |
| 22 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 23 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 24 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 25 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
| A1: Premium still-SR Gate 14 selector smoke | open, local | Execute the persisted selector sidecar through the actual render/model-loading path before any long train. | `premium_still_sr_gate14_selector_smoke_<date>` | The real selector path reproduces the Gate 14 intake pass, rejects schema/checkpoint drift, uses no REF/source/JPEG/gate metrics, and decides whether promotion is allowed. |
| A2: Premium still-SR promotion | open after selector smoke | After selector smoke passes, produce a no-REF candidate-only model receipt that clears the 15% / 15% held-out floor, then run `check_premium_still_sr_promotion_gate.py` and `check_production_capture_submission.py`. | `premium_still_sr_promotion_receipts` | Full 50 MP / 100 MP routed gates pass, worst-row recovery is nonnegative, raw-editor latitude/openability opens for every route, timing/memory/checkpoint hashes exist, exact-sidecar-only noise policy passes, and `check_production_capture_submission.py` passes. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate 10 classifies the blocker as `source_degradation_target_mismatch`: X2D target distribution is `3.4500243590744026x` the same-camera train median and Z8 is mostly noise-floor targets (`28/36`). Gate 11 route-isolated teacher/router smoke fails, Gate 12 paired smoke fails, and single-source Gate 13 gating fails per scene. Gate 14 intake now closes the upper-bound-to-sidecar step: seven rules, six source mappings, 78 runtime-safe features, X2D sidecar replay median/worst MAE `0.329828330762138%` / `0.0%` and `0.02786331921791634%` / `0.0%`, and Z8 exact-noop `0.0%` / `0.0%`. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not start a long run from rejected single-source branches. The next local action is `premium_still_sr_gate14_selector_smoke_<date>`: run the persisted selector sidecar through the actual model-loading/render path, reject schema/checkpoint drift, and prove it reproduces the Gate 14 intake pass before promotion. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
