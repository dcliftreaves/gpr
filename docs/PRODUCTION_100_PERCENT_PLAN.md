...
| 4 | in progress | Close the route-specialist Premium Still/SR blockers: refresh routed full-frame gates, add rendered/editor-latitude receipts for every route, wire exact-sidecar-only noise policy into target construction, then build a production submission. | `premium_still_sr_promotion_receipts` accepted by `check_production_capture_submission.py`. |
...
| 4 | Premium still-SR promotion | Refresh routed full-frame gates, add rendered/editor-latitude receipts for every route, wire exact-sidecar-only noise policy, then build the production submission. | `premium_still_sr_promotion_receipts` and production submission audit. | Mark Premium still/SR 100% only if the production checker passes; otherwise record the exact blocker class. |
...
|---|---|---|---|---|
| A: Premium still-SR promotion | open, local | Continue from `/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_route_readiness_20260702/index.html`. The route-specialist path is the next production direction because it has route coverage and positive full-frame metric floors for Mission 1 50 MP DNG/GPR, Z8 50 MP DNG, and X2D 100 MP DNG; the clean-source split is rejected for long training because its paired smoke passes X2D but fails Z8. Next command class: refresh routed full-frame gates, build rendered/editor-latitude receipts, wire exact-sidecar-only noise policy, then run `check_production_capture_submission.py`. | `premium_still_sr_promotion_receipts` | Full 50 MP / 100 MP routed gates pass, worst-row recovery is nonnegative, rendered/editor-latitude review opens for every route, timing/memory/checkpoint hashes exist, exact-sidecar-only noise policy passes, and `check_production_capture_submission.py` passes. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 99 runtime-safe receipts and 0 promotable receipts. The clean-source split smoke receipts narrow one rejected path: X2D passes the short gate at `+0.008822244790194556%` median MAE and `+0.003090454316737371%` median RMSE recovery, while Z8 fails at `-0.07358643668344858%` median MAE and `-0.01893710250797187%` median RMSE. The route-specialist readiness audit now identifies the next production direction: Mission 1 50 MP DNG/GPR, Z8 50 MP DNG, and X2D 100 MP DNG have route coverage and positive full-frame metric floors. | Do not start a long run from the source-evidence split candidate. Continue the route-specialist/raw-CFA path by refreshing full-frame routed gates, adding rendered/editor-latitude receipts, wiring exact-sidecar-only noise policy, and building the production submission. | The routed candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes exact-sidecar-only noise policy, and passes production submission validation. |
