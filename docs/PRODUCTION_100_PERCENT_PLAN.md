...
candidate intake and selector smoke are now closed, so the next command must
produce the full 50 MP / 100 MP promotion receipts or update that promotion
receipt with a precise blocker. Do not start optional video SR, PSF research, or
new dashboard cosmetics while Gate A has a local next command.
...
| 22 | open | Build the strict Premium still-SR promotion receipt from the Gate 14 routed selector, route readiness, editor/openability, noise policy, formal promotion gate, and production-capture requirements; then run the full 50 MP / 100 MP validation that the receipt identifies as missing. | `premium_still_sr_promotion_receipts` pass the 15% / 15% held-out MAE/RMSE floor and production submission checker, or classify the exact blocker. |
| 23 | open | Package the Mission 1/iPhone noise-sidecar capture request around strict provenance only; do not promote candidate dark-looking frames. | Capture/provenance packet listing exact missing Mission/iPhone darkframes and validation commands. |
| 24 | open | When true no-scene-signal Mission/iPhone darkframes exist, build sidecars with `build_camera_noise_calibration.py --require-source-provenance`. | `mission1_darkframe_stack` and `iphone_cfa_darkframe_stack` both validate. |
| 25 | external | Keep the Mission 1 camera-role runbook ready for GoPro/Mission 1 firmware owners; no local Pi stand-in can close this gate. | Real camera-role source/storage/display receipts with 120+ sustained frames, zero drops, valid `.gvid`, and 20+ fps source/encode/preview. |
| 26 | closed/protect | Keep approved 4K cleanup and 8K SR evidence locked. Run only lock-ledger/readme/manifest guards unless a locked artifact fails. | Product lock ledger, README pillar guard, and release manifest guard pass. |
...
|---|---|---|---|
| Premium still/SR | Open. The current scoreboard has 124 runtime-safe receipts and 0 promotable receipts. Gate16 paired smoke passed on a narrow branch, but the all-target-row audit rejects it for production promotion: `463` X2D/100 MP target-tile rows, no 50 MP rows, no full-frame scope, `-0.12226915231999792%` median MAE recovery, `-0.1296250122706981%` median RMSE recovery, and `-9.625700832601128%` worst-row MAE recovery. Gate17 supplies a balanced target package with `576` 50 MP rows and `576` 100 MP rows, candidate-only runtime policy, and no production claim. The first Gate17 trained checkpoint is rejected: `1152` target rows, `-0.23468499188533842%` overall median MAE, `0.34200684333480724%` overall median RMSE, `-35.30304893327897%` worst 100 MP MAE, and `-2.259351982942634%` worst 50 MP MAE. Gate18 is also rejected: it improves tail safety but collapses toward no-op with `0.0%` overall median MAE/RMSE, `-0.0912221669777865%` worst 100 MP MAE, and `-0.0068758277986793615%` worst 50 MP MAE. Gate19 source-HF direct prediction is rejected: `-14.17838003215098%` overall median MAE, `-13.181758464778333%` overall median RMSE, and severe tails. Gate17 scalar-direction calibration is rejected: best scalar `0.025`, `0.017883242885033075%` median MAE recovery, negative worst row. The route-specialist readiness audit identifies the production route surface, editor/latitude coverage is 4/4 ready, and the clean-signal target noise policy passes, but no supplied model receipt clears the 15% / 15% promotion floor. | Do not rerun Gate16, the same Gate17 training command, Gate18 no-op, Gate19 source-HF direct prediction, or scalar tuning as the production candidate. Build Gate20 to recover positive signal with a changed supervision/objective, run broad target-row audit with both 50 MP and 100 MP evidence, then run full promotion validation with timing/memory, editor/openability, exact-sidecar-only noise policy, and production submission validation. | The replacement candidate beats the still baseline on 50 MP and 100 MP holdouts, has nonnegative worst-row recovery, records timing/memory/checkpoint hashes, passes raw-editor latitude/openability for all four required routes, passes exact-sidecar-only noise policy, and passes production submission validation. |
