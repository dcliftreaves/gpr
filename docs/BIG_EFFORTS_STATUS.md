...
| Raw video improvement / PSF-aware Bayer resize | 45% | Partly done through 4K cleanup, candidate-aware 8K SR, native high/low pair inventory, measurement plan, first native measurement run, kernel-stability audit, a hash-strict PSF gap closure plan, and metric-bearing same-cell detail reruns for Mission42/Z8 baseline/candidate summaries. | Not done as a formal PSF/blur-calibrated resizing model until controlled high/low pairs with source hashes, decoded Bayer hashes, fixed settings, and negative controls produce a stable native kernel, and a PSF-conditioned model beats the approved baseline on Mission42/Z8 same-cell detail plus the existing worst-row gates. |
