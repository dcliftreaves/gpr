...
- `tools/cnn/train_premium_still_sr_raw_cfa_residual.py` now trains a
  ...
  A matched global-context U-Net on the same regenerated deduplicated
  CFA-aware target reaches about 0.149 percent median raw MAE recovery and
  about -0.0049 percent median RMSE recovery, so it improves RMSE versus the
  0.153 percent noise-floor baseline but still trails on MAE and is not
  promotable:
  ...
  A non-box PSF/CFA NAF diagnostic using explicit known-kernel weights
  `[0.52, 0.23, 0.17, 0.08]` reaches about 0.130 percent median raw MAE
  recovery and about 0.0025 percent median RMSE recovery. It also trails the
  0.153 percent small U-Net branch, so fixed global PSF conditioning is not the
  still-SR fix; real row-level PSF/camera variation remains the needed input:
  ...
  recovery on the hard X2D holdout. A matched 1200-step global-context pass on
  the current X2D-scene/noise-floor baseline reaches about 0.149 percent
  median MAE recovery and about -0.0049 percent RMSE recovery, still below the
  0.153 percent small U-Net branch on MAE. This class of candidate-only
  raw-context CNN is now narrowed further; the next attempt needs a different runtime
