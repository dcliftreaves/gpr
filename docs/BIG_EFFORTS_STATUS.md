...
- `tools/build_premium_still_sr_experiment_scoreboard.py` now ranks the
  ...
  The current scoreboard scans 124 runtime-safe rendered-HF, raw-CFA residual,
  clean-signal, clean-source pair, frequency-pyramid, gated-residual, and masked-detail
  ...
  produce positive held-out recovery; the masked-detail/no-op branch also
  fails X2D/Z8 and route-local scene smokes. This confirms the current 60
  percent pillar score is still appropriate and narrows the next pass to a
  materially different target/source or teacher objective, not another
  architecture-only residual tweak or masked-threshold sweep.
