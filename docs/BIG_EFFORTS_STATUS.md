...
| Raw video improvement / PSF-aware Bayer resize | 48% | Partly done through 4K cleanup, candidate-aware 8K SR, native high/low pair inventory, measurement plan, first native measurement run, kernel-stability audit, a hash-strict PSF gap closure plan, metric-bearing same-cell detail reruns for Mission42/Z8 baseline/candidate summaries, a PSF-detail-aware SR scoreboard, a Mission gradient/detail blocker audit, and a focused hard-row continuation that improves Mission/Z8 medians. | Not done as a formal PSF/blur-calibrated resizing model until controlled high/low pairs with source hashes, decoded Bayer hashes, fixed settings, and negative controls produce a stable native kernel, and a PSF-conditioned model beats the approved baseline on Mission42/Z8 same-cell detail plus the existing worst-row gates. The current focused candidate still misses the Mission worst-row floor on `GP017346`, `GP017600`, `GP017347`, and `GP017348`. |
