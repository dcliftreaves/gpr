...
| 4 | Premium still/SR Gate 14 selector smoke | open/local | Run the persisted sidecar through the actual selector/model-loading render path, then compare the sidecar replay against the Gate 14 intake receipt. | `premium_still_sr_gate14_selector_smoke_<date>` reproduces the X2D pass, preserves Z8 exact-noop, records model/checkpoint hashes, and uses no REF/source/JPEG/gate metric inputs. |
| 5 | Premium still/SR promotion | open/local after selector smoke | Run the full 50 MP / 100 MP promotion gate only after the selector smoke passes. | `premium_still_sr_promotion_receipts` pass 15% / 15% held-out MAE/RMSE, nonnegative worst-row MAE, editor/openability, timing/memory, checkpoint hashes, and exact-sidecar-only noise policy. |
...
Gate 14 candidate intake passed as executable-selector preflight, not as
production. The receipt is:

`/Volumes/OWC_8TB/gpr_work/artifacts/premium_still_sr_gate14_candidate_intake_20260702/candidate_preflight.json`

The pass means this:

- The Gate 13 upper bound is now persisted as a deterministic seven-rule
  first-match selector sidecar with six source mappings and 78 candidate-only
  runtime features.
- It clears both X2D scenes with zero negative selected rows through sidecar
  replay: `x2d_2025_austin_06` median MAE `0.329828330762138%`, worst `0.0%`;
  `x2d_2025_austin_07` median MAE `0.02786331921791634%`, worst `0.0%`.
- Z8 remains exact no-op at `0.0%` median and `0.0%` worst-row MAE.
- `long_run_allowed=false`; the selector has not yet been run through the actual
  model-loading/render path.

The next step is therefore not another broad CNN run. It is Gate 14 selector
smoke: execute the persisted sidecar through the actual render/model-loading
path and prove it reproduces the intake receipt without REF, source RAW/RGB/HF,
JPEG, or gate metrics at render time.
