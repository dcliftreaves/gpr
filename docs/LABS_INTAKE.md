...
| Half-res 24 fps capture target | Latest strict Pi 5 stand-in receipt is blocked at 19.98 fps median versus 24 fps target; the corrected pixel-format short direct `.gvid` probe is also blocked at 19.85 fps; the best short luma-pair near-miss is 23.54 fps and remains below target | `docs/LABS_TARGET_BENCH.md`, `docs/VIDEO_STATUS.md` |
...
| Can it hit the half-res capture-rate target on the stand-in path? | Not on the latest strict receipt: commit `0dd6660` writes 14,400/14,400 frames with 0 drops and valid `.gvid`, but reaches only 19.98 fps median versus the 24 fps target. Commit `e16357f` fixed the bench path so pixel format 4 reaches the encoder context; the corrected 120-frame direct `.gvid` probe still reaches only 19.85 fps median. A later scratch luma-pair probe reached 23.54 fps on a short direct-container run, but still missed the target and was not committed. |
...
1. Restore the half-res Pi 5 path to >= 24 fps. Follow-up probes narrowed the
   ...
   historical-doc commits both run below target today, the corrected
   pixel-format direct `.gvid` probe misses at 19.85 fps, producer unpack is
   now guarded for decimated capture, quality/quant sweeps top out below target,
