...
| Premium still/SR | 60% | Still-SR datasets, raw-CFA targets, routed specialists, editor-openability receipts, exact-sidecar-only noise policy, promotion tooling, and 118 runtime-safe experiment receipts. | A no-REF 50 MP / 100 MP candidate that beats the current still baseline and clears the promotion gate. Current best older runtime-safe recovery is 4.03% MAE / 3.75% RMSE against the 15% / 15% floor; frequency-pyramid fails X2D worst-row/Z8 median and worst-row gates, and gated no-op residual reaches near-parity but not positive held-out recovery. |
