...
## Implemented Format And Proposed Changes

The fork already writes **GVID v1**. Its authoritative layout is defined in
[`gpr_video_format.h`](../source/lib/vc5_encoder/gpr_video_format.h), with
[conformance checks](GVID_CONFORMANCE.md). Use that format for current camera
evaluation and host playback: a 32-byte clip header, a 16-byte frame header,
`fps_x1000`, and length-delimited frame payloads. Rendering metadata is carried
in a sidecar; the current wire format has no payload CRC or final index.

The tables below propose a future container revision. Rational frame rates,
in-band metadata, payload CRCs, variable headers, and indexes are **not
implemented by the current writer or reader**. Files using those layouts will
not work with the current tools. Adopting structural changes requires a new
container version, matching reader/writer implementations, and conformance
tests. The existing v1 format remains the integration target for this PR.
