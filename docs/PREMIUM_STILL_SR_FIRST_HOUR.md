...
| What should happen before another long CNN run? | Continue the route-specialist/raw-CFA lane first: build true raw-editor latitude/openability receipts for every route, wire exact-sidecar-only noise policy, and build a production submission. Do not scale the current Restormer, teacher-first, window-attention, or source-evidence split clean-source pair setup unless both smoke holdouts beat interpolation. |
