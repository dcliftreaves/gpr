...
| What is the current scorecard state? | **99** runtime-safe training receipts, **0** promotable rows, and best older runtime-safe recovery of **4.03%** MAE / **3.75%** RMSE against the **15% / 15%** promotion floor. The newest launchable window-attention smoke gate also fails before long training: X2D is **+0.0016%** median MAE but **-0.0095%** RMSE, and Z8 is **-0.5807%** MAE / **-0.3829%** RMSE. The source-evidence split smoke pass rejects that long-run direction: X2D passes at **+0.0088%** median MAE / **+0.0031%** median RMSE, but Z8 fails at **-0.0736%** median MAE / **-0.0189%** median RMSE. The route-specialist readiness audit is now the next production lane: Mission 1 50 MP DNG/GPR, Z8 50 MP DNG, and X2D 100 MP DNG have route coverage and positive full-frame metric floors, but still need rendered/editor-latitude review, exact-sidecar-only noise policy, and production submission validation. |
...
| What should happen before another long CNN run? | Continue the route-specialist/raw-CFA lane first: refresh routed full-frame gates, add rendered/editor-latitude receipts for every route, wire exact-sidecar-only noise policy, and build a production submission. Do not scale the current Restormer, teacher-first, window-attention, or source-evidence split clean-source pair setup unless both smoke holdouts beat interpolation. |
